...
const s = {
  ...
  btn:         { width: '100%', padding: '10px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '600', background: '#6366f1', color: '#fff', marginTop: '10px' },
...
export default function Popup() {
  ...
  useEffect(() => {
    // Ask content script if this page is a job application
    ...
    // Check auth state
    chrome.storage.local.get(['user'], (res) => setIsLoggedIn(!!res.user))
  }, [])
    ...
    <div style={s.wrap}>
      ...
      {!isLoggedIn && (
        <div style={s.card}>
          ...
          <button style={s.btn} onClick={handleLogin}>Sign in with Google</button>
        </div>
      )}
